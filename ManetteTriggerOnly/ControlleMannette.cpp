...
void RunProcedure2(SOCKET socket)
{
	...
	int Left_AnalogValue;
	int Right_AnalogValue;
	int valToSend;
	...
	if (vr_pointer->PollNextEvent(&event, sizeof(event)))
	{
		...
		case k_EButton_SteamVR_Trigger:
			...
			case VREvent_ButtonPress:
				...
				//detection manette gauche
				else if (role == TrackedControllerRole_LeftHand) {
					//print "left" pour visualiser la manette utilisée
					std::cout << "Left\n";
					vr_pointer->GetControllerStateWithPose(TrackingUniverseStanding, TrackedControllerRole_LeftHand, &controllerState, sizeof(controllerState), &trackedDevicePose);
					int t = 1;
					while (controllerState.rAxis[t].x != 0) {
						//Nouveau
						vr_pointer->GetControllerStateWithPose(TrackingUniverseStanding, TrackedControllerRole_LeftHand, &controllerState, sizeof(controllerState), &trackedDevicePose);
						t = 1;
						Left_AnalogValue = 300 + (controllerState.rAxis[t].x * 254);
				...
				//detection manette droite
				else if (role == TrackedControllerRole_RightHand)
				{
					//print "right" pour visualiser la manette utilisée
					std::cout << "right\n";
					vr_pointer->GetControllerStateWithPose(TrackingUniverseStanding, TrackedControllerRole_RightHand, &controllerState, sizeof(controllerState), &trackedDevicePose);
					int t = 1;
					while (controllerState.rAxis[t].x != 0) {
						//Nouveau
						vr_pointer->GetControllerStateWithPose(TrackingUniverseStanding, TrackedControllerRole_RightHand, &controllerState, sizeof(controllerState), &trackedDevicePose);
						t = 1;
					    Right_AnalogValue = 1+(controllerState.rAxis[t].x*254);
